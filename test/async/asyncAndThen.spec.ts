...
import { ok, err, asyncAndThen } from '../../src/index.js';

describe('asyncAndThen', () => {
    it('chains async success (curried)', async () => {
        const chain = asyncAndThen(async (x: number) => ok(x * 2));
    ...
    it('passes through failure', async () => {
        const r = await asyncAndThen(async (x: number) => ok(x * 2), err<string>('fail'));
        expect(r.isFailure).toBe(true);
        if (r.isFailure) expect(r.error).toBe('fail');
    });

    it('propagates async callback failure', async () => {
        const r = await asyncAndThen(async () => err<string>('inner'), ok(21));
        expect(r.isFailure).toBe(true);
        if (r.isFailure) expect(r.error).toBe('inner');
    });

    it('catches callback exceptions', async () => {
        const r = await asyncAndThen(async () => { throw 'cb err'; }, ok(21));
        expect(r.isFailure).toBe(true);
