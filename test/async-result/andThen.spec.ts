...
import { andThen } from '../../src/async-result/andThen.js';
...
describe('AsyncResult andThen', () => {
    ...
    it('short-circuits on failure', async () => {
        const ar = andThen((x: number) => fromResult(err<string>('nested')), fromResult(err<string>('fail')));
        const result = await ar.run();
    ...
    it('is curried', async () => {
        const chain = andThen((x: number) => fromResult(ok(x + 1)));
