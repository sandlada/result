...
describe('combine', () => {
    ...
    it('short-circuits on the first failure and returns it', () => {
        ...
        const combined = combine([ok(1), err<Error>(error), ok(3)]);
    ...
    it('short-circuits on a middle failure', () => {
        ...
        const combined = combine([
            ok(1),
            err<Error>(middleErr),
            err<Error>(new Error('never seen')),
        ]);
        expect(combined.isSuccess).toBe(false);
        if (!combined.isSuccess) {
    ...
    it('first-error short-circuit on homogeneous mixed success/failure (Group C)', () => {
        // Brief: combine([ok(1), err('a'), ok('x')]) returns err('a') — when A and E
...
describe('fp/combine', () => {
    ...
    it('single failure → returns that error (Group C)', () => {
        const result = combine<number, string>([err('only')]);
        expect(result.isSuccess).toBe(false);
        if (!result.isSuccess) expect(result.error).toBe('only');
    });
});