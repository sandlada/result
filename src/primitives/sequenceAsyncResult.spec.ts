...
describe('sequenceAsyncResult', () => {
    ...
    it('returns Ok([]) when the input is empty (boundary)', async () => {
        const r = await sequenceAsyncResult([]).run();
        expect(r.isSuccess).toBe(true);
        if (r.isSuccess) expect(r.value).toEqual([]);
    });

    it('preserves error object identity on short-circuit (first-error wins)', async () => {
    ...
    it('returns a single-element array on a single success (boundary)', async () => {
        const r = await sequenceAsyncResult([fromResult(ok(42))]).run();
        expect(r.isSuccess).toBe(true);
        if (r.isSuccess) expect(r.value).toEqual([42]);
    });

    it('does not invoke later thunks after a short-circuit (laziness & short-circuit)', async () => {
        let laterCalls = 0;
        const r = await sequenceAsyncResult([
            fromResult(ok(1)),
    ...
    it('awaiting the produced AsyncResult yields IResultOfT<T[], E> (promise semantics)', async () => {
    ...
    it('does not eagerly resolve inner thunks at construction (laziness)', () => {
        let constructed = 0;
    ...
    it('last-position failure short-circuits correctly (last-position short-circuit)', async () => {
        const r = await sequenceAsyncResult([
            fromResult(ok(1)),
            fromResult(ok(2)),
