...
import { any, type AnyError } from './any.js';
import { fromResult } from '../async-result/index.js';
import { ok, err } from '../factories/index.js';
import type { AsyncResult } from '../types/AsyncResult.js';

describe('any types', () => {
    it('returns AsyncResult<T[], AnyError<E>[]>', () => {
        const r = any([fromResult(ok(1)), fromResult(err('a'))]);
        const _check: AsyncResult<number[], AnyError<string>[]> = r;
        expectTypeOf(_check).toBeObject();
    });

    it('infers T and E from inputs', () => {
        const r = any<boolean, number>([]);
        const _check: AsyncResult<boolean[], AnyError<number>[]> = r;
        expectTypeOf(_check).toBeObject();
    });
    ...
    it('preserves literal error types as an AnyError<E>[] array', () => {
        type Err = 'upstream' | 'downstream';
        const r = any<number, Err>([fromResult(ok(1))]);
        const _check: AsyncResult<number[], AnyError<Err>[]> = r;
        expectTypeOf(_check).toEqualTypeOf<AsyncResult<number[], AnyError<Err>[]>>();
    });
